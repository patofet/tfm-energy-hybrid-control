"""MPC+RL amb macro-pas de 4 hores (6 decisions/dia)."""
import os
import sys
...
MACRO_STEP_MIN = 240                          # 4 hores
EPISODE_LEN    = MINS_IN_DAY // MACRO_STEP_MIN  # 6 decisions/dia
N_STEPS        = EPISODE_LEN * 80             # 480 — divisible per 96
BATCH_SIZE     = 96                           # 480/96 = 5 minibatches


if __name__ == "__main__":
    print("Pre-procesando datos (MPC+RL 4h)...")
    ...
    print("Instanciando Agente PPO (Macro-Step 4h)...")
    model = PPO(
        "MlpPolicy",
        env,
        verbose=1,
    ...
    model.set_logger(Logger(folder=None, output_formats=[
        LiveOutputFormat(),
        CSVOutputFormat(os.path.join(LOCAL_LOGS_DIR, "logs_mpcrl_4h.csv")),
    ]))

    ruta_guardado = os.path.join(os.path.dirname(os.path.abspath(__file__)), "modelo_mpc_rl_bess_4h")
