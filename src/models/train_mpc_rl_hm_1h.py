"""MPC+RL amb macro-pas 1h i MPC tracker horari (solve/hora)."""
import os
import sys
...
EPISODE_LEN          = MINS_IN_DAY // MACRO_STEP_MIN  # 24 decisions/dia
N_STEPS              = EPISODE_LEN * 80               # 1920 — divisible per 96
BATCH_SIZE           = 96                             # 1920/96 = 20 minibatches


if __name__ == "__main__":
    print("Pre-procesando datos (MPC+RL hm 1h)...")
    ...
    model.set_logger(Logger(folder=None, output_formats=[
        LiveOutputFormat(),
        CSVOutputFormat(os.path.join(LOCAL_LOGS_DIR, "logs_mpcrl_hm_1h.csv")),
    ]))

    ruta_guardado = os.path.join(os.path.dirname(os.path.abspath(__file__)), "modelo_mpc_rl_bess_hm_1h")
