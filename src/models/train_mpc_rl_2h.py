"""MPC+RL amb macro-pas de 2 hores (12 decisions/dia)."""
import os
import sys
...
if __name__ == "__main__":
    ...
    print("Instanciando Agente PPO (Macro-Step 2h)...")
    model = PPO(
        "MlpPolicy",
        env,
        verbose=1,
    ...
    model.set_logger(Logger(folder=None, output_formats=[
        LiveOutputFormat(),
        CSVOutputFormat(os.path.join(LOCAL_LOGS_DIR, "logs_mpcrl_2h.csv")),
    ]))

    ruta_guardado = os.path.join(os.path.dirname(os.path.abspath(__file__)), "modelo_mpc_rl_bess_2h")
