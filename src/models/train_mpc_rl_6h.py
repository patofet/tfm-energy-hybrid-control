"""MPC+RL amb macro-pas de 6 hores (4 decisions/dia)."""
import os
import sys
...
MACRO_STEP_MIN = 360                           # 6 hores
EPISODE_LEN    = MINS_IN_DAY // MACRO_STEP_MIN # 4 decisions/dia
N_STEPS        = EPISODE_LEN * 80              # 320 — divisible per 32
BATCH_SIZE     = 32                            # 320/32 = 10 minibatches


if __name__ == "__main__":
    print("Pre-procesando datos (MPC+RL 6h)...")
    ...
    print("Instanciando Agente PPO (Macro-Step 6h)...")
    model = PPO(
        "MlpPolicy",
        env,
        verbose=1,
    ...
    model.set_logger(Logger(folder=None, output_formats=[
        LiveOutputFormat(),
        CSVOutputFormat(os.path.join(LOCAL_LOGS_DIR, "logs_mpcrl_6h.csv")),
    ]))

    ruta_guardado = os.path.join(os.path.dirname(os.path.abspath(__file__)), "modelo_mpc_rl_bess_6h")
