"""MPC+RL amb macro-pas 90min i MPC tracker horari (solve/hora)."""
import os
import sys
...
if __name__ == "__main__":
    print("Pre-procesando datos (MPC+RL hm 90min)...")
    ...
    model.set_logger(Logger(folder=None, output_formats=[
        LiveOutputFormat(),
        CSVOutputFormat(os.path.join(LOCAL_LOGS_DIR, "logs_mpcrl_hm_90min.csv")),
    ]))

    ruta_guardado = os.path.join(os.path.dirname(os.path.abspath(__file__)), "modelo_mpc_rl_bess_hm_90min")
