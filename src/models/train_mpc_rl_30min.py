"""MPC+RL amb macro-pas de 30 minuts (48 decisions/dia)."""
import os
import sys
...
MACRO_STEP_MIN = 30                            # 30 minuts
EPISODE_LEN    = MINS_IN_DAY // MACRO_STEP_MIN # 48 decisions/dia
N_STEPS        = EPISODE_LEN * 80              # 3840 — divisible per 96
BATCH_SIZE     = 96                            # 3840/96 = 40 minibatches


if __name__ == "__main__":
    print("Pre-procesando datos (MPC+RL 30min)...")
    ...
    print("Instanciando Agente PPO (Macro-Step 30min)...")
    model = PPO(
        "MlpPolicy",
        env,
        verbose=1,
    ...
    model.set_logger(Logger(folder=None, output_formats=[
        LiveOutputFormat(),
        CSVOutputFormat(os.path.join(LOCAL_LOGS_DIR, "logs_mpcrl_30min.csv")),
    ]))

    ruta_guardado = os.path.join(os.path.dirname(os.path.abspath(__file__)), "modelo_mpc_rl_bess_30min")
