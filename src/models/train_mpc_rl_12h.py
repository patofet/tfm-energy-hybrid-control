"""MPC+RL amb macro-pas de 12 hores (2 decisions/dia)."""
import os
import sys
...
MACRO_STEP_MIN = 720                           # 12 hores
EPISODE_LEN    = MINS_IN_DAY // MACRO_STEP_MIN # 2 decisions/dia
N_STEPS        = EPISODE_LEN * 80              # 160 — divisible per 32
BATCH_SIZE     = 32                            # 160/32 = 5 minibatches


if __name__ == "__main__":
    print("Pre-procesando datos (MPC+RL 12h)...")
    ...
    print("Instanciando Agente PPO (Macro-Step 12h)...")
    model = PPO(
        "MlpPolicy",
        env,
        verbose=1,
    ...
    model.set_logger(Logger(folder=None, output_formats=[
        LiveOutputFormat(),
        CSVOutputFormat(os.path.join(LOCAL_LOGS_DIR, "logs_mpcrl_12h.csv")),
    ]))

    ruta_guardado = os.path.join(os.path.dirname(os.path.abspath(__file__)), "modelo_mpc_rl_bess_12h")
