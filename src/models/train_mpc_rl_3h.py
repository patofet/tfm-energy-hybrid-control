"""MPC+RL amb macro-pas de 3 hores (8 decisions/dia)."""
import os
import sys
...
MACRO_STEP_MIN = 180                           # 3 hores
EPISODE_LEN    = MINS_IN_DAY // MACRO_STEP_MIN # 8 decisions/dia
N_STEPS        = EPISODE_LEN * 80              # 640 — divisible per 64
BATCH_SIZE     = 64                            # 640/64 = 10 minibatches


if __name__ == "__main__":
    print("Pre-procesando datos (MPC+RL 3h)...")
    ...
    print("Instanciando Agente PPO (Macro-Step 3h)...")
    model = PPO(
        "MlpPolicy",
        env,
        verbose=1,
    ...
    model.set_logger(Logger(folder=None, output_formats=[
        LiveOutputFormat(),
        CSVOutputFormat(os.path.join(LOCAL_LOGS_DIR, "logs_mpcrl_3h.csv")),
    ]))

    ruta_guardado = os.path.join(os.path.dirname(os.path.abspath(__file__)), "modelo_mpc_rl_bess_3h")
